...
public class OutcomeTests
{
    ...
    [Fact]
    public void Should_CreateProblemOutcome_FromToOutcomeExtension() => Assert.Equal(new Outcome<None>(TestProblem), TestProblem.ToOutcome());

    [Fact]
    public void Should_CreateStronglyTypedProblemOutcome_FromToOutcomeExtension() => Assert.Equal(new Outcome<int>(TestProblem), TestProblem.ToOutcome<int>());

    [Fact]
    public void Should_CreateProblemOutcome_FromProblemHelper() => Assert.Equal(new Outcome<None>(TestProblem), Outcome.Problem(TestProblem));

    [Fact]
    public void Should_CreateStronglyTypedProblemOutcome_ProblemHelper() => Assert.Equal(new Outcome<int>(TestProblem), TestProblem.Problem<int>());
    ...
    {
        var outcome = TestProblem.ToOutcome();
    ...
    {
        var invoked = false;
        _ = Outcome.Ok.OnSuccess(_ => invoked = true);
        Assert.True(invoked);
    }
    ...
    {
        var invoked = false;
        _ = new Outcome<None>(TestProblem).OnSuccess(_ => invoked = true);
    ...
    {
        var invoked = false;
        _ = await Task.FromResult(Outcome.Ok).OnSuccessAsync(_ => invoked = true);
        Assert.True(invoked);

        invoked = false;
        _ = await ValueTask.FromResult(Outcome.Ok).OnSuccessAsync(_ => invoked = true);
        Assert.True(invoked);
    }
    ...
    {
        var problem = TestProblem.ToOutcome();
        var invoked = false;
        _ = await Task.FromResult(problem).OnSuccessAsync(_ => invoked = true);
        Assert.False(invoked);

        _ = await ValueTask.FromResult(problem).OnSuccessAsync(_ => invoked = true);
        Assert.False(invoked);
    }
    ...
    {
        var invoked = false;
        _ = new Outcome<None>(TestProblem).OnProblem(_ => invoked = true);
        Assert.True(invoked);
    }
    ...
    {
        var invoked = false;
        _ = Outcome.Ok.OnProblem(_ => invoked = true);
        Assert.False(invoked);
    }
    ...
    public async Task OnProblemAsync_ShouldBeInvoked_WhenThereIsAProblem()
    {
        var problem = TestProblem.ToOutcome();
        var invoked = false;
        _ = await Task.FromResult(problem).OnProblemAsync(_ => invoked = true);
        Assert.True(invoked);

        invoked = false;
        _ = await ValueTask.FromResult(problem).OnProblemAsync(_ => invoked = true);
        Assert.True(invoked);
    }
    ...
    {
        var invoked = false;
        _ = await Task.FromResult(Outcome.Ok).OnProblemAsync(_ => invoked = true);
        Assert.False(invoked);

        _ = await ValueTask.FromResult(Outcome.Ok).OnProblemAsync(_ => invoked = true);
        Assert.False(invoked);
    }
    ...
    {
        var expected = TestProblem.ToOutcome<int>();
